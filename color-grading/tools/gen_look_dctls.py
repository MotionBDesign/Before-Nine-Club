#!/usr/bin/env python3
"""tools/gen_look_dctls.py -- generate resolve/dctl/looks/BNC_L01_Clean.dctl
through BNC_L13_Airy.dctl from tools/bnc_looks.LOOKS.

Deterministic and idempotent: re-running this script regenerates every look
DCTL byte-for-byte from the current contents of tools/bnc_looks.py, so it is
safe to re-run any time the recipes in that file are re-tuned. It writes only
under resolve/dctl/looks/ and never reads or writes anything else.

DCTL cannot reliably #include across files, so the full set of look-toolbox
helper functions (spec sections 4-5: Oklab conversions, gamut_clip and every
op) is kept once, here, as HELPERS_BLOCK, and copied verbatim into every
generated file -- the same block that is hand-copied into
resolve/dctl/BNC_Look_Toolbox.dctl. If you change one, change the other.

Usage: python3 tools/gen_look_dctls.py [--out-dir resolve/dctl/looks]
"""
...
import bnc_looks  # noqa: E402  (path insert above must run first)

# ---------------------------------------------------------------------------
# Shared helper block (spec sections 4-5) -- single source of truth for the
# C code copied into every generated look DCTL. Keep in sync by hand with
# the identical block hand-copied into resolve/dctl/BNC_Look_Toolbox.dctl.
# ---------------------------------------------------------------------------
HELPERS_BLOCK = r"""// ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Op -> C call plumbing. Every constructor in bnc_color.py resolves its own
# defaults into Op.kwargs at call time (they are plain Python functions with
# default arguments), so every kwargs dict in bnc_looks.LOOKS already has
# every key below present -- no fallback defaulting is needed here.
# ---------------------------------------------------------------------------
OP_ARG_ORDER = {
    "exposure": ["ev"],
    "white_balance": ["warm", "tint"],
    "contrast": ["amount", "pivot", "hi", "lo"],
    "lift_blacks": ["amount"],
    "toe": ["strength", "range"],
    "shoulder": ["start", "k"],
    "softclip": ["hi", "lo"],
    ...
    "vibrance": ["amount"],
    "density": ["k"],
    "hue_shift": ["centre", "width", "shift"],
    "hue_sat": ["centre", "width", "mult"],
    "hue_lum": ["centre", "width", "mult"],
    "bw_mix": ["wr", "wg", "wb"],
    "tint": ["hue", "amount"],
}
...
//
// BNC Grade look "{look.name}" ({look.id}) -- GENERATED FILE, do not hand-edit.
// Produced by tools/gen_look_dctls.py from the "{look.id}" entry in
...
{HELPERS_BLOCK}
// ---------------------------------------------------------------------------
// The "{look.name}" recipe (spec section 5, in order -- see header above).
// ---------------------------------------------------------------------------
...
def _wrap_comment(text: str, width: int = 74) -> str:
    """Wrap `text` into '// '-prefixed lines for a header comment block."""
    import textwrap
...
def main() -> None:
    ...
    default_out = os.path.normpath(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "resolve", "dctl", "looks")
    )
    parser.add_argument("--out-dir", default=default_out, help="output directory for the generated .dctl files")
    args = parser.parse_args()

    os.makedirs(args.out_dir, exist_ok=True)

    written = []
    for look in bnc_looks.LOOKS:
        filename = f"BNC_{look.id}_{look.slug}.dctl"
        path = os.path.join(args.out_dir, filename)
        content = render_look_file(look)
        with open(path, "w") as f:
            f.write(content)
        written.append((filename, len(look.ops)))

    print(f"Generated {len(written)} look DCTLs in {args.out_dir}:")
    for filename, n_ops in written:
        print(f"  {filename:32s} {n_ops} ops")
