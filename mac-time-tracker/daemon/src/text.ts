...
const GENERIC = new Set([
  ...
  'v', 'ver', 'version', 'final', 'finalfinal', 'draft', 'wip', 'copy', 'new', 'old', 'backup', 'bak',
  'rev', 'revision', 'amend', 'amends', 'approved', 'master', 'temp', 'tmp', 'untitled',
  // folder furniture
  'clients', 'client', 'projects', 'project', 'work', 'jobs', 'job', 'assets', 'artwork', 'files',
  'working', 'output', 'exports', 'export', 'links', 'footage', 'source', 'shared', 'general',
  'documents', 'desktop', 'downloads', 'dropbox', 'volumes', 'users', 'library',
  // generic app words
  'the', 'and', 'for', 'with', 'from', 'a', 'of', 'to', 'in', 'on', 'at',
]);
...
/** Tokens worth matching on: noise, years and month names removed. */
export function tokenize(input: string): string[] {
  const out: string[] = [];
  for (const token of rawTokens(input)) {
    if (GENERIC.has(token) || MONTHS.has(token)) continue;
...
/**
 * Weighted overlap of query tokens against a target, normalised by the target's
 * own weight so short task names aren't unfairly punished. Returns 0..1.
 */
export function overlapScore(query: string[], target: string[], idf: Map<string, number>): number {
  ...
  let matched = 0;
  let total = 0;
  for (const token of uniq(target)) {
    const weight = idf.get(token) ?? fallback;
    total += weight;
    if (querySet.has(token)) matched += weight;
  }
